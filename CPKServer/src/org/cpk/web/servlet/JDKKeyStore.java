...
import org.cpk.web.servlet.JCEBlockCipher.PBEWithSHAAndDES3Key;
    ...
    implements BCKeyStore
{
    ...
    private class StoreEntry
    {
            ...
            throws Exception
        {
            ...
            JCEBlockCipher.PBEWithSHAAndDES3Key cipherSpi = makePBECipher_1(KEY_CIPHER, Cipher.ENCRYPT_MODE, password, salt, iterationCount);
...
//            dOut = new DataOutputStream(cOut);
            ...
            dOut1.close();
            byte[] toBeEnc = bOut1.toByteArray();
            
            byte[] enced = cipherSpi.engineDoFinal(toBeEnc, 0, toBeEnc.length);
    ...
    protected JCEBlockCipher.PBEWithSHAAndDES3Key makePBECipher_1(
            String  algorithm,
            int     mode,
            char[]  password,
            byte[]  salt,
            ...
            throws IOException
        {
            try
            {
                PBEKeySpec          pbeSpec = new PBEKeySpec(password);
//                SecretKeyFactory    keyFact = SecretKeyFactory.getInstance(algorithm/*, "BC"*/);
                JCESecretKeyFactory.PBEWithSHAAndDES3Key keyFact = new JCESecretKeyFactory.PBEWithSHAAndDES3Key();
                PBEParameterSpec    defParams = new PBEParameterSpec(salt, iterationCount);

                JCEBlockCipher.PBEWithSHAAndDES3Key cipherSpi = new JCEBlockCipher.PBEWithSHAAndDES3Key();
//                Cipher cipher = Cipher.getInstance(algorithm/*, "BC"*/);

                cipherSpi.engineInit(mode, keyFact.engineGenerateSecret(pbeSpec), defParams, new SecureRandom());
